...
@Named
@ViewScoped
public class MayoristasMensual implements Serializable {

    private boolean estado;
    private ListadoEstablecimientoModel arrayDeJson;
    private TokenModel token;
    private List<establecimientoModel> listadoEstablecimientos;
    private List<establecimientoModel> filterEstablecimientos;
    private String usuario = "MMARVIN";
    private int inicio;
    private int xfin;
    private int anio;
    ...
    public void consumidor() {
        try {
            ...
            arrayDeJson = new ListadoEstablecimientoModel();
            arrayDeJson = (ListadoEstablecimientoModel) services.consumidorDerecursoPorTokenGet(parametros, "http://"+Const.rutaServ+":8080/Ws-oauth2-gs/resources/reportes/reporteMay",
                    "Bearer " + token.getAccessToken(), arrayDeJson);
            listadoEstablecimientos = arrayDeJson.getReporteMay();
            if (token != null) {
...
public StreamedContent onImprime() throws JRException, IOException {
        ...
        InputStream jasperReport = ((ServletContext) FacesContext.getCurrentInstance().getExternalContext().getContext()).getResourceAsStream("/reports/rpt_establecimientos.jasper");
